...
class ApexBot {
  ...
  async dealRequset(url: string, param?: any) {
    if (this.requestPromise.length < this.maxRequestLen)
      ...
      throw new Error('每秒只能请求2个')
    console.warn(`请求${this.serializeParamsWithUrl(url, param)},参数为:${JSON.stringify(param)}`)
    const res = await request(this.serializeParamsWithUrl(url, param))
    this.requestPromise.splice(this.requestPromise.findIndex(item => item === url), 1)
  ...
  async queryPlayerByName(player: string, platForm?: string) {
    if (player) {
      try {
        ...
        const res: any = await this.dealRequset('/bridge', { player, platform })
  ...
  async queryMap() {
    try {
      ...
      const content = `当前匹配地图:${mapCnName[currentBr.map]}\n地图持续时间:${formatTime(currentBr.start)} - ${formatTime(currentBr.end)}\n即将轮换地图:${
      mapCnName[nextBr.map]
    }\n地图持续时间:${formatTime(nextBr.start)} - ${formatTime(nextBr.end)}\n\n当前排位地图:${mapCnName[currentRk.map]}\n地图持续时间:地图持续时间:${formatTime(currentRk.start)} - ${formatTime(currentRk.end)}\n明日排位地图:${
      mapCnName[nextRk.map]
    }\n地图持续时间:地图持续时间:${formatTime(nextRk.start)} - ${formatTime(nextRk.end)}`
      console.warn(content)
      return content
    }
    catch (error) {
    ...
    try {
      ...
      Object.keys(RP).forEach((key) => {
        if (RP[key].foundRank !== -1) {
          const contentItem = `${key}端\n猎杀底分：${RP[key].val}\n更新时间：${moment(Number(RP[key].updateTimestamp) * 1000).format('YYYY-MM-DD HH:mm:ss')}\n大师猎杀总人数：${RP[key].totalMastersAndPreds}`
  ...
  async queryUidByName(player: string, platForm?: string) {
    if (player) {
      try {
        const platform = platForm || 'PC'
        const res: any = await this.dealRequset('/nametouid', { player, platform })
